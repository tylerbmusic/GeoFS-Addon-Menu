...
window.GMenu = class {
	//The 'G' stands for either GeoFS or GGamerGGuy, depending on how big you think my ego is. I put the class in the window scope for easy access.
	...
	initialize() {
		...
		if (!window.gmenu.menuDiv) {
			...
			window.gmenu.menuDiv.classList =
	...
	addButton(title, fn, options) {
		this.html +=
			options == undefined
				? `<button id="${this.prefix}${title}">${title}</button>`
				: `<button id="${this.prefix}${title}" ${options}>${title}</button>`;
