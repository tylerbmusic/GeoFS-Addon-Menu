...
setTimeout(function () {
	...
	window.softLanding = new Audio(
		"https://tylerbmusic.github.io/GPWS-files_geofs/soft_landing.wav"
	);
	window.hardLanding = new Audio(
		"https://tylerbmusic.github.io/GPWS-files_geofs/hard_landing.wav"
	);
	window.crashLanding = new Audio(
		"https://tylerbmusic.github.io/GPWS-files_geofs/crash_landing.wav"
	);
	...
	window.statsDiv.style.background =
		"linear-gradient(to bottom right, rgb(29, 52, 87), rgb(20, 40, 70))";
	...
	window.statsDiv.style.boxShadow =
	...
	function updateLndgStats() {
		if (geofs.cautiousWithTerrain == false && !geofs.isPaused()) {
			...
			if (ldgAGL < 500) {
				window.justLanded =
					geofs.animation.values.groundContact && !window.isGrounded;
                ...
                <button style="
                    ...
                    <div class="info-block">
                        ...
                        <span>G-Forces: ${(
													geofs.animation.values.accZ / 9.80665
												).toFixed(2)}G</span>
                        <span>Terrain-calibrated V/S: ${window.calVertS.toFixed(
													1
												)}</span>
                        <span>True airspeed: ${window.kTrue} kts</span>
                        <span>Ground speed: ${window.groundSpeed.toFixed(
													1
												)} kts</span>
                        <span>Indicated speed: ${window.ktias} kts</span>
                        <span>Roll: ${geofs.animation.values.aroll.toFixed(
													1
												)} degrees</span>
                        <span>Tilt: ${geofs.animation.values.atilt.toFixed(
													1
												)} degrees</span>
                ...
                `;
					...
					if (geofs.nav.units.NAV1.inRange) {
                        ...
                        <div style="margin-top: 10px; font-size: 14px;">
                            ...
                            <span>Deviation from center: ${geofs.nav.units.NAV1.courseDeviation.toFixed(
															1
														)}</span>
                        </div>`;
					}
					if (Number(window.vertSpeed) < 0) {
						...
						if (Number(window.vertSpeed) >= -50) {
							...
							Number(window.vertSpeed) >= -500 &&
							Number(window.vertSpeed) < -200
						) {
				...
				if (geofs.nav.units.NAV1.inRange) {
					window.isInTDZ =
						geofs.nav.units.NAV1.distance * FEET_TO_METERS >
							0.052902913939976676 *
	...
	function updateCalVertS() {
		if (
			...
			window.calVertS =
				(window.newAGL - window.oldAGL) *
				(60000 / (window.newTime - window.oldTime));
			window.oldAGL =
				geofs.animation.values.altitude !== undefined &&
				geofs.animation.values.groundElevationFeet !== undefined
					? geofs.animation.values.altitude -
